...
class ZohoAttendanceBotFirefox {
    constructor() {
        this.init();
    }
    ...
    init() {
        ...
        ZohoAttendanceShared.log('Firefox Attendance Bot initialized');
    }

    setupMessageListener() {
        browser.runtime.onMessage.addListener((request, sender) => {
            if (request.action === 'performAttendance') {
                return this.performAttendance(request.type)
                    .then(result => result)
    ...
    async findAndClickByAria(type) {
        ...
        for (const label of ariaLabels) {
            // Firefox-compatible attribute selector
            const element = document.querySelector(`[aria-label*="${label}"], [aria-labelledby*="${label}"]`);
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => new ZohoAttendanceBotFirefox());
} else {
    new ZohoAttendanceBotFirefox();
}